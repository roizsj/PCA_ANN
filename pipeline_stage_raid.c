...
static uint64_t g_stage0_replica_base_lba = 0;

static int worker_disk_index(const stage_worker_t *w)
{
    if (!w || !w->app || !w->disk) {
        return -1;
    }
    for (int i = 0; i < NUM_STAGES; i++) {
        if (w->disk == &w->app->disks[i]) {
            return i;
        }
    }
    return -1;
...
static uint64_t stage0_replica_base_lba(const ivf_meta_t *meta)
{
    uint64_t end_lba = 0;
    if (!meta || !meta->clusters) {
        return 0;
    }
    if (g_stage0_replica_base_lba != 0) {
        return g_stage0_replica_base_lba;
    }
    end_lba = meta->header.base_lba;
    for (uint32_t c = 0; c < meta->nlist; c++) {
        uint64_t cluster_end = meta->clusters[c].start_lba + meta->clusters[c].num_lbas;
        if (cluster_end > end_lba) {
            end_lba = cluster_end;
        }
    }
    g_stage0_replica_base_lba = end_lba;
    return g_stage0_replica_base_lba;
}
...
static int submit_vec_bundle_range(stage_worker_t *w, struct bundle_run *run)
{
    ...
    int disk_idx = worker_disk_index(w);

    /*
     * RAID stage0 layout:
     *   - disk0 stores shard0 in the normal flex region.
     *   - disks1..3 store their normal shard in the normal flex region.
     *   - disks1..3 store a shard0 replica in a replica region immediately
     *     after the normal cluster region.
     *
     * So a stage0 worker bound to disk1..3 must translate normal LBA X to:
     *   replica_base_lba + (X - base_lba)
     */
    if (w->stage_id == 0 && disk_idx > 0) {
        uint64_t replica_base = stage0_replica_base_lba(meta);
        lba = replica_base + (lba - meta->header.base_lba);
    }
...
static void *stage_worker_main(void *arg) {
    ...
    if (app->max_read_lbas > 0 && max_bundles_per_read > app->max_read_lbas) {
        max_bundles_per_read = app->max_read_lbas;
    }
    if (w->lane_id == 0) {
        fprintf(stderr,
                "[stage %d] read_depth=%u max_read_lbas=%u gap_merge=%u disk=%s\n",
                w->stage_id,
                read_depth,
    ...
    while (1) {
        ...
        const uint32_t max_gap_bundles = app->stage1_gap_merge_limit;
...
)
{
    ...
    fprintf(stderr,
            "[pipeline_init] begin read_depth=%u gap_merge=%u active_stages=%u coarse_backend=%s prune_threshold_mode=%s ivf_meta=%s sorted_ids=%s\n",
            read_depth,
    ...
    fprintf(stderr, "[pipeline_init] RAID stage0 enabled: stage0 workers are striped over all disks; disk1..3 read shard0 from replica region\n");
    ...
    for (int s = 0; s < NUM_STAGES; s++) {
        ...
        for (uint32_t lane = 0; lane < worker_count; lane++) {
            ...
            /*
             * Stage0 reads the replicated first shard. Spread stage0 workers
             * across all disks so entry batches can use disk0 normal shard0
             * and disk1..3 shard0 replicas.
             */
            if (s == 0) {
                w->disk = &app->disks[lane % NUM_STAGES];
            } else {
                w->disk = &app->disks[s];
            }
            w->qpair = NULL;
