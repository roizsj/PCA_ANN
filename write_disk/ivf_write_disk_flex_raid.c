...
#define NUM_SHARDS 4
#define MAGIC_META 0x49564634u /* "IVF4"; stage0 replica layout keeps same metadata format */
...
static void usage(const char *prog) {
    fprintf(stderr,
            ...
            "  - RAID mode writes shard0 normally to disk0 and also replicates shard0 to disks1..3\n"
            "    after the normal region. The search side derives replica_base_lba as\n"
            "    base_lba + total_cluster_lbas.\n",
            prog);
}
...

    {
...

        {
...

            {
                ...
                uint64_t total_cluster_lbas = cur_lba - app.cfg.base_lba;
                uint64_t replica_base_lba = app.cfg.base_lba + total_cluster_lbas;

                fprintf(stderr, "[info] total_lbas_to_write=%" PRIu64 "\n",
                        total_cluster_lbas);
                fprintf(stderr,
                        "[info] stage0 replica_base_lba=%" PRIu64
                        " on disk1,disk2,disk3 replica_lbas=%" PRIu64 "\n",
                        replica_base_lba,
                        total_cluster_lbas);
                ...
                for (uint32_t c = 0; c < cents.nlist; c++) {
                    ...
                    for (uint32_t b = 0; b < num_lbas; b++) {
...

                        {
                            ...
                            /*
                             * Stage0 RAID replica:
                             *   - disk0 keeps shard0 in the normal region at lba.
                             *   - disks1..3 keep their normal shards at lba.
                             *   - disks1..3 also get shard0 at replica_base_lba + offset.
                             *
                             * The metadata is intentionally unchanged. The RAID search path
                             * computes the replica LBA from base_lba and total cluster LBAs.
                             */
                            memset(ctx, 0, sizeof(ctx));
                            uint64_t replica_lba = replica_base_lba + (lba - app.cfg.base_lba);
                            for (int s = 1; s < NUM_SHARDS; s++) {
                                ...
                                if (rc != 0) {
                                    fprintf(stderr,
                                            "replica write submit failed: disk=%d lba=%" PRIu64
                                            " rc=%d\n",
                                            s, replica_lba, rc);
                                    exit(EXIT_FAILURE);
                                }
