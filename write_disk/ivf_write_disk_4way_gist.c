...
#define NUM_SHARDS 4
#define MAGIC_META 0x49563443u                    // "IV4C": full clusters assigned to 4 disks
...
typedef struct {
    uint32_t cluster_id;
    uint32_t disk_id;       // cluster 整体落在哪个盘
...
static void init_spdk_and_attach(AppState *app) {
    ...
    opts.name = "ivf_cluster_4way_writer_spdk";
...
static void save_meta_file(const char *path,
    ...
    for (uint32_t i = 0; i < nlist; i++) {
        ...
        fwrite(&cluster_meta[i].disk_id, sizeof(cluster_meta[i].disk_id), 1, fp);
...
int main(int argc, char **argv) {
    ...
    /*
     * Step 3: 以 cluster 为单位分配到 4 个盘。
     * 同一个 cluster 不拆开；贪心选择当前累计向量数最少的盘。
     */
    ClusterMeta *cluster_meta = (ClusterMeta *)xcalloc(cents.nlist, sizeof(ClusterMeta));
    uint64_t cur_lba[NUM_SHARDS];
    uint64_t disk_vec_load[NUM_SHARDS] = {0};
    uint64_t disk_lba_load[NUM_SHARDS] = {0};
    for (int s = 0; s < NUM_SHARDS; s++) {
        cur_lba[s] = app.cfg.base_lba;
    }
    ...
    for (uint32_t c = 0; c < cents.nlist; c++) {
        ...
        for (int s = 1; s < NUM_SHARDS; s++) {
            if (disk_vec_load[s] < disk_vec_load[best_disk]) {
                best_disk = s;
            }
        }

        cluster_meta[c].cluster_id = c;
        cluster_meta[c].disk_id = (uint32_t)best_disk;
        cluster_meta[c].start_lba = cur_lba[best_disk];
        cluster_meta[c].num_vectors = sz;
        cluster_meta[c].num_lbas = num_lbas;
        cur_lba[best_disk] += num_lbas;
        disk_vec_load[best_disk] += sz;
        disk_lba_load[best_disk] += num_lbas;
    }

    for (int s = 0; s < NUM_SHARDS; s++) {
        fprintf(stderr,
                "[info] disk%d clusters/vector load=%" PRIu64 " vectors, %" PRIu64 " lbas\n",
                s, disk_vec_load[s], disk_lba_load[s]);
    }
    ...
    /*
     * Step 4: 每个盘准备 1 个可复用 DMA buffer
     */
    ...
    for (uint32_t c = 0; c < cents.nlist; c++) {
        ...
        for (uint32_t b = 0; b < num_lbas; b++) {
            ...
            for (uint32_t lane = 0; lane < vectors_per_lba; lane++) {
                ...
                uint8_t *dst = dma_buf[disk_id] + (size_t)lane * shard_bytes;
            ...
            int rc = spdk_nvme_ns_cmd_write(
                app.disks[disk_id].ns,
                app.disks[disk_id].qpair,
                dma_buf[disk_id],
                lba,
                1,
                io_complete,
            ...
            if (rc != 0) {
                fprintf(stderr, "write submit failed: disk=%u lba=%" PRIu64 " rc=%d\n",
                        disk_id, lba, rc);
                exit(EXIT_FAILURE);
            }

            wait_io(app.disks[disk_id].qpair, &ctx);
        }
