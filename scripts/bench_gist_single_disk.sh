...
META="${META:-${OUT_DIR}/ivf_meta_1_disk.bin}"
SORTED_IDS="${SORTED_IDS:-${OUT_DIR}/sorted_vec_ids_1_disk.bin}"
...
MAKE_TARGETS="${MAKE_TARGETS:-ivf_write_disk_1 ivf_baseline_2_gist}"

mkdir -p "${RESULT_DIR}"
TS="$(date +%Y%m%d_%H%M%S)"
CSV="${RESULT_DIR}/gist_single_disk_${TS}.csv"
LOG_DIR="${RESULT_DIR}/gist_single_disk_${TS}_logs"
mkdir -p "${LOG_DIR}"
...
if [[ "${RUN_WRITE}" != "0" ]]; then
  echo "[info] writing one-disk GIST IVF layout to ${DISK0}"
  "${BIN_DIR}/ivf_write_disk_1" \
    --input "${BASE_FVECS}" \
    --centroids "${CENTROIDS}" \
    --codebook "${CODEBOOK}" \
    --meta "${META}" \
