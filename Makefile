...
CFLAGS := -std=c11 -O3 -march=native -g -Wall -Wextra -Wpedantic -Wno-unused-parameter           -D_GNU_SOURCE           -I$(SPDK_DIR)/include $(SPDK_CFLAGS)
CXXFLAGS := -std=c++17 -O3 -march=native -g -Wall -Wextra -Wno-unused-parameter             -D_GNU_SOURCE             -I$(SPDK_DIR)/include -I$(FAISS_DIR)/include $(SPDK_CFLAGS)
...
ivf_write_disk: ivf_write_disk.c
	$(CC) $(CFLAGS) -o $(BIN_DIR)/$@ ivf_write_disk.c -pthread 	-Wl,--whole-archive -Wl,-Bstatic $(SPDK_LIBS) 	-Wl,-Bdynamic -Wl,--no-whole-archive $(SPDK_SYSLIBS)

ivf_write_disk_flex: ivf_write_disk_flex.c
	$(CC) $(CFLAGS) -o $(BIN_DIR)/$@ ivf_write_disk_flex.c -pthread 	-Wl,--whole-archive -Wl,-Bstatic $(SPDK_LIBS) 	-Wl,-Bdynamic -Wl,--no-whole-archive $(SPDK_SYSLIBS)

ivf_write_disk_1: ivf_write_disk_1.c
	$(CC) $(CFLAGS) -o $(BIN_DIR)/$@ ivf_write_disk_1.c -pthread 	-Wl,--whole-archive -Wl,-Bstatic $(SPDK_LIBS) 	-Wl,-Bdynamic -Wl,--no-whole-archive $(SPDK_SYSLIBS)
