...
RAID_APP_NAME := pca_ann_raid
RAID_APP_C_SRCS := 	main_raid.c 	pipeline_stage_raid.c 	query_loader.c
RAID_APP_OBJS := $(addprefix build/obj/,$(RAID_APP_C_SRCS:.c=.o)) $(addprefix build/obj/,$(APP_CXX_SRCS:.cpp=.o))
...
all: $(APP_NAME) $(RAID_APP_NAME) ivf_write_disk ivf_write_disk_flex ivf_write_disk_flex_raid ivf_write_disk_overlap_flex ivf_write_disk_1 ivf_write_disk_raid_gist ivf_write_disk_4way_gist ivf_pq_write_disk_gist ivf_baseline_1 ivf_baseline_2 ivf_baseline_2_gist ivf_baseline_4way_gist ivf_baseline_4way_raid_gist ivf_baseline_4way_raid_gist_pipeline ivf_pq_baseline_gist
...
$(RAID_APP_NAME): $(RAID_APP_OBJS)
	@mkdir -p $(BIN_DIR)
	$(CC) -o $(BIN_DIR)/$@ $(RAID_APP_OBJS) -pthread $(FAISS_LDFLAGS) $(FAISS_LIBS) 	-Wl,--whole-archive -Wl,-Bstatic $(SPDK_LIBS) 	-Wl,-Bdynamic -Wl,--no-whole-archive $(SPDK_SYSLIBS)
...
ivf_write_disk_flex_raid: write_disk/ivf_write_disk_flex_raid.c
	@mkdir -p $(BIN_DIR)
	$(CC) $(CFLAGS) -o $(BIN_DIR)/$@ write_disk/ivf_write_disk_flex_raid.c -pthread 	-Wl,--whole-archive -Wl,-Bstatic $(SPDK_LIBS) 	-Wl,-Bdynamic -Wl,--no-whole-archive $(SPDK_SYSLIBS)
...
ivf_write_disk_raid_gist: write_disk/ivf_write_disk_raid_gist.c
	@mkdir -p $(BIN_DIR)
	$(CC) $(CFLAGS) -o $(BIN_DIR)/$@ write_disk/ivf_write_disk_raid_gist.c -pthread 	-Wl,--whole-archive -Wl,-Bstatic $(SPDK_LIBS) 	-Wl,-Bdynamic -Wl,--no-whole-archive $(SPDK_SYSLIBS)
...
ivf_baseline_2_gist: baseline/ivf_baseline_2_gist.c query_loader.c $(OBJ_DIR)/coarse_search_faiss.o
	@mkdir -p $(BIN_DIR)
	$(CC) $(CFLAGS) -o $(BIN_DIR)/$@ baseline/ivf_baseline_2_gist.c query_loader.c $(OBJ_DIR)/coarse_search_faiss.o -lm -pthread $(FAISS_LDFLAGS) $(FAISS_LIBS) 	-Wl,--whole-archive -Wl,-Bstatic $(SPDK_LIBS) 	-Wl,-Bdynamic -Wl,--no-whole-archive $(SPDK_SYSLIBS)

ivf_baseline_4way_gist: baseline/ivf_baseline_4way_gist.c query_loader.c $(OBJ_DIR)/coarse_search_faiss.o
	@mkdir -p $(BIN_DIR)
	$(CC) $(CFLAGS) -o $(BIN_DIR)/$@ baseline/ivf_baseline_4way_gist.c query_loader.c $(OBJ_DIR)/coarse_search_faiss.o -lm -pthread $(FAISS_LDFLAGS) $(FAISS_LIBS) 	-Wl,--whole-archive -Wl,-Bstatic $(SPDK_LIBS) 	-Wl,-Bdynamic -Wl,--no-whole-archive $(SPDK_SYSLIBS)

ivf_baseline_4way_raid_gist: baseline/ivf_baseline_4way_raid_gist.c query_loader.c $(OBJ_DIR)/coarse_search_faiss.o
	@mkdir -p $(BIN_DIR)
	$(CC) $(CFLAGS) -o $(BIN_DIR)/$@ baseline/ivf_baseline_4way_raid_gist.c query_loader.c $(OBJ_DIR)/coarse_search_faiss.o -lm -pthread $(FAISS_LDFLAGS) $(FAISS_LIBS) 	-Wl,--whole-archive -Wl,-Bstatic $(SPDK_LIBS) 	-Wl,-Bdynamic -Wl,--no-whole-archive $(SPDK_SYSLIBS)

ivf_baseline_4way_raid_gist_pipeline: baseline/ivf_baseline_4way_raid_gist_pipeline.c query_loader.c $(OBJ_DIR)/coarse_search_faiss.o
	@mkdir -p $(BIN_DIR)
	$(CC) $(CFLAGS) -o $(BIN_DIR)/$@ baseline/ivf_baseline_4way_raid_gist_pipeline.c query_loader.c $(OBJ_DIR)/coarse_search_faiss.o -lm -pthread $(FAISS_LDFLAGS) $(FAISS_LIBS) 	-Wl,--whole-archive -Wl,-Bstatic $(SPDK_LIBS) 	-Wl,-Bdynamic -Wl,--no-whole-archive $(SPDK_SYSLIBS)
...
clean:
	rm -f $(BIN_DIR)/$(APP_NAME) $(BIN_DIR)/$(RAID_APP_NAME) $(BIN_DIR)/ivf_write_disk $(BIN_DIR)/ivf_write_disk_flex $(BIN_DIR)/ivf_write_disk_flex_raid $(BIN_DIR)/ivf_write_disk_overlap_flex $(BIN_DIR)/ivf_write_disk_1 $(BIN_DIR)/ivf_write_disk_raid_gist $(BIN_DIR)/ivf_write_disk_4way_gist $(BIN_DIR)/ivf_pq_write_disk_gist $(BIN_DIR)/ivf_baseline_1 $(BIN_DIR)/ivf_baseline_2 $(BIN_DIR)/ivf_baseline_2_gist $(BIN_DIR)/ivf_baseline_4way_gist $(BIN_DIR)/ivf_baseline_4way_raid_gist $(BIN_DIR)/ivf_baseline_4way_raid_gist_pipeline $(BIN_DIR)/ivf_pq_baseline_gist
	rm -rf $(OBJ_DIR)
