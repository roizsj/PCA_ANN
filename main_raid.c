...
static int parse_runtime_options(int argc, char **argv, runtime_options_t *opts)
{
    ...
    static struct option long_opts[] = {
        ...
        {"gap-merge", required_argument, 0, 'g'},
        {"stage1-gap-merge", required_argument, 0, 'g'},
        {"max-read-lbas", required_argument, 0, 'L'},
    ...
    while ((opt = getopt_long(argc, argv, "", long_opts, &idx)) != -1) {
        switch (opt) {
            ...
            case 'g':
                ...
                break;
            case 'L':
                opts->max_read_lbas = (uint32_t)strtoul(optarg, NULL, 10);
                break;
            case 'a':
            ...
            default:
                fprintf(stderr,
                        "Usage: %s [--max-queries N] [--nprobe N] [--read-depth N] [--gap-merge N] [--max-read-lbas N] [--active-stages N] [--coarse-backend brute|faiss] [--prune-threshold-mode centroid|sampled] [--iova-mode pa|va] [--print-per-query] [--summary-only]\n",
                        argv[0]);
                return -1;
        }
    }
    ...
    if (opts->stage1_gap_merge_limit > MAX_BATCH) {
        fprintf(stderr, "parse_runtime_options: gap-merge must be in [0, %d]\n", MAX_BATCH);
        return -1;
    }
    if (opts->max_read_lbas > MAX_BATCH) {
        fprintf(stderr, "parse_runtime_options: max-read-lbas must be in [0, %d]\n", MAX_BATCH);
        return -1;
    }
...
int main(int argc, char **argv)
{
    ...
    opts.name = "spdk_4stage_ann_raid";
    opts.mem_size = 1024;
    ...
    const char *ivf_meta_path =
        "/home/zhangshujie/ann_ssd/pca_ann/preprocessing/gist1m_output/ivf_meta_raid.bin";
    ...
    const char *sorted_ids_path =
        "/home/zhangshujie/ann_ssd/pca_ann/preprocessing/gist1m_output/sorted_ids_raid.bin";
    ...
    app.max_read_lbas = runtime_opts.max_read_lbas;
