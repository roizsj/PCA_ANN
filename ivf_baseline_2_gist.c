    ...
    *bundles_read = 0;
    ...
    for (uint32_t h = 0; h < nprobe; h++) {
        ...
        while (remaining_lbas > 0) {
            ...
            if (read_lba_range(ns, qpair, dma_buf, cur_lba, chunk_lbas) != 0) {
                fprintf(stderr,
                        "read_lba_range failed cluster=%u lba=%" PRIu64 " count=%u\n",
                        ci->cluster_id,
                        cur_lba,
                        chunk_lbas);
                return -1;
...
static int parse_args(int argc, char **argv, Config *cfg)
{
    ...
    if (!cfg->disk_traddr || cfg->nprobe == 0 || cfg->threads == 0) {
        print_usage(argv[0]);
        return -1;
    }
...
int main(int argc, char **argv)
{
    ...
    if (max_queries > 0 && worker_count > max_queries) {
        worker_count = max_queries;
    }
    if (worker_count == 0) {
        worker_count = 1;
    }
    ...
    for (uint32_t i = 0; i < worker_count; i++) {
        ...
        worker->ctrlr = disk.ctrlr;
        worker->ns = disk.ns;
        worker->sector_size = disk.sector_size;
...
join_workers:
    ...
    if (queries_ran > 0) {
        ...
        printf("[baseline2 summary] queries=%u nprobe=%u threads=%u total_wall_ms=%.3f qps=%.3f avg_latency_ms=%.3f avg_coarse_ms=%.3f avg_io_ms=%.3f avg_compute_ms=%.3f avg_candidates=%.1f avg_bundles=%.1f avg_recall@%d=%.4f\n",
               queries_ran,
               cfg.nprobe,
               worker_count,
        ...
        printf("[baseline2 summary] no queries completed\n");
        fflush(stdout);
    }
