...
#define TOPK 10
#define NUM_SHARDS 4
#define MAGIC_META 0x49563443u

static const char *DEFAULT_META_PATH = "/home/zhangshujie/ann_ssd/pca_ann/preprocessing/gist1m_output/ivf_meta_4way_cluster.bin";
static const char *DEFAULT_SORTED_IDS_PATH = "/home/zhangshujie/ann_ssd/pca_ann/preprocessing/gist1m_output/sorted_vec_ids_4way_cluster.bin";
...
static int parse_ivf_meta(const char *path, IvfMeta *meta)
{
    ...
    if (meta->header.dim == 0 || meta->header.num_shards != NUM_SHARDS) {
        fprintf(stderr, "parse_ivf_meta: invalid layout dim=%u num_shards=%u expected_shards=%u\n",
                meta->header.dim, meta->header.num_shards, NUM_SHARDS);
        fclose(fp);
        return -1;
    }
    ...
    for (uint32_t i = 0; i < meta->nlist; i++) {
        ...
        if (fread(&cm.cluster_id, sizeof(cm.cluster_id), 1, fp) != 1 ||
            fread(&cm.disk_id, sizeof(cm.disk_id), 1, fp) != 1 ||
        ...
        if (cm.disk_id >= NUM_SHARDS) {
            fprintf(stderr, "parse_ivf_meta: invalid disk id cluster=%u disk=%u\n",
                    cm.cluster_id, cm.disk_id);
            free_ivf_meta(meta);
            fclose(fp);
            return -1;
        }
        ...
        meta->clusters[i].disk_id = cm.disk_id;
...
static int read_lba_range(struct spdk_nvme_ns *ns,
                          struct spdk_nvme_qpair *qpair,
                          void *buf,
                          uint64_t lba,
                          uint32_t lba_count)
...
static int run_single_query(QueryWorker *worker, uint32_t query_idx)
{
...
static void *query_worker_main(void *arg)
{
    ...
    if (worker->init_rc == 0) {
        for (;;) {
            ...
            pthread_mutex_lock(&worker->dispatch->mu);
            if (worker->dispatch->fatal_rc != 0 || worker->dispatch->next_query_idx >= worker->max_queries) {
                pthread_mutex_unlock(&worker->dispatch->mu);
                break;
            }
            query_idx = worker->dispatch->next_query_idx++;
            pthread_mutex_unlock(&worker->dispatch->mu);

            if (run_single_query(worker, query_idx) != 0) {
                pthread_mutex_lock(&worker->dispatch->mu);
...
int main(int argc, char **argv)
{
    ...
    opts.name = "ivf_baseline_query_parallel";
    opts.mem_size = 1024;
...
join_workers:
    ...
    if (queries_ran > 0) {
        ...
        printf("[baseline4way-cluster summary] queries=%u nprobe=%u threads=%u total_wall_ms=%.3f qps=%.3f avg_latency_ms=%.3f avg_coarse_ms=%.3f avg_io_ms=%.3f avg_compute_ms=%.3f avg_candidates=%.1f avg_bundles=%.1f avg_recall@%d=%.4f\n",
               queries_ran,
               cfg.nprobe,
               worker_count,
        ...
        printf("[baseline4way-cluster summary] no queries completed\n");
        fflush(stdout);
    }
