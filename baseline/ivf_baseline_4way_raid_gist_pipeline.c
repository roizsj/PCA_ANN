...
typedef struct PreparedTask PreparedTask;
struct PreparedTask {
    PreparedTask *next;
    uint32_t query_idx;
    uint32_t disk_id;
...
static int coarse_search_hits(const float *query,
                              const IvfMeta *meta,
                              uint32_t nprobe,
                              bool use_faiss_coarse,
                              coarse_search_module_t *coarse_module,
                              ...
                              float *distances_buf)
{
    if (!query || !meta || !hits_buf) {
        return -1;
    }

    if (!use_faiss_coarse) {
        return coarse_search_topn(query, meta, nprobe, hits_buf);
    }

    if (!coarse_module || !labels_buf || !distances_buf) {
        return -1;
    }

    int rc = coarse_search_module_search(coarse_module, query, nprobe, labels_buf, distances_buf);
    if (rc != 0) {
        return rc;
    }

    for (uint32_t i = 0; i < nprobe; i++) {
        hits_buf[i].cluster_id = labels_buf[i];
        hits_buf[i].dist = distances_buf[i];
    }
    return 0;
}
...
static int task_queue_init(TaskQueue *queue)
{
    ...
    if (pthread_mutex_init(&queue->mu, NULL) != 0) {
        perror("pthread_mutex_init queue");
        return -1;
    }
    if (pthread_cond_init(&queue->cv, NULL) != 0) {
        perror("pthread_cond_init queue");
...
static void task_queue_destroy(TaskQueue *queue)
{
    ...
    PreparedTask *cur = queue->head;
    while (cur) {
        PreparedTask *next = cur->next;
        free(cur);
        cur = next;
    }
...
static void task_queue_fail(TaskQueue *queue, int rc)
{
...
static int task_queue_push(TaskQueue *queue, PreparedTask *task)
{
    if (!queue || !task) {
        return -1;
    }
    ...
    task->next = NULL;
    pthread_mutex_lock(&queue->mu);
    if (queue->fatal_rc != 0 || queue->closed) {
        pthread_mutex_unlock(&queue->mu);
        return -1;
    }
    if (queue->tail) {
...
static PreparedTask *task_queue_pop(TaskQueue *queue, int *fatal_rc_out)
{
    if (fatal_rc_out) {
        *fatal_rc_out = 0;
    }
    ...
    pthread_mutex_lock(&queue->mu);
    while (!queue->head && !queue->closed && queue->fatal_rc == 0) {
        pthread_cond_wait(&queue->cv, &queue->mu);
    }

    if (queue->fatal_rc != 0) {
        if (fatal_rc_out) {
            *fatal_rc_out = queue->fatal_rc;
        }
        pthread_mutex_unlock(&queue->mu);
        return NULL;
    }

    PreparedTask *task = queue->head;
    if (task) {
...
static void task_queue_close(TaskQueue *queue)
{
    pthread_mutex_lock(&queue->mu);
    queue->closed = true;
    pthread_cond_broadcast(&queue->cv);
...
static int start_barrier_init(StartBarrier *barrier, uint32_t total_threads)
{
    if (!barrier) {
        return -1;
    }
    ...
    if (pthread_mutex_init(&barrier->mu, NULL) != 0) {
        perror("pthread_mutex_init barrier");
        return -1;
    }
    if (pthread_cond_init(&barrier->cv, NULL) != 0) {
        perror("pthread_cond_init barrier");
...
static void start_barrier_signal_ready(StartBarrier *barrier, int init_rc)
{
    pthread_mutex_lock(&barrier->mu);
    if (init_rc != 0 && barrier->fatal_rc == 0) {
        barrier->fatal_rc = init_rc;
    }
    barrier->ready_threads++;
    pthread_cond_broadcast(&barrier->cv);
    while (!barrier->start && barrier->fatal_rc == 0) {
        pthread_cond_wait(&barrier->cv, &barrier->mu);
    }
    pthread_mutex_unlock(&barrier->mu);
}

static int start_barrier_wait_ready(StartBarrier *barrier)
{
    pthread_mutex_lock(&barrier->mu);
    while (barrier->ready_threads < barrier->total_threads && barrier->fatal_rc == 0) {
        pthread_cond_wait(&barrier->cv, &barrier->mu);
    }
    int fatal_rc = barrier->fatal_rc;
    barrier->start = true;
    pthread_cond_broadcast(&barrier->cv);
    pthread_mutex_unlock(&barrier->mu);
    return fatal_rc;
...
static void start_barrier_fail(StartBarrier *barrier, int rc)
{
    pthread_mutex_lock(&barrier->mu);
    if (barrier->fatal_rc == 0) {
        barrier->fatal_rc = rc;
    }
    barrier->start = true;
    pthread_cond_broadcast(&barrier->cv);
...
static int run_prepared_query(IoWorker *worker, const PreparedTask *task)
{
    if (!worker || !task) {
        return -1;
    }

    QueryResult *res = &worker->results[task->query_idx];
    memset(res, 0, sizeof(*res));
    res->rc = -1;
    res->disk_id = task->disk_id;
    res->coarse_us = task->coarse_us;

    const float *query = worker->queries + (size_t)task->query_idx * (size_t)worker->query_dim;
    TopkState topk = {0};
    ...
    res->latency_us = now_us() - (task->enqueue_ts_us - task->coarse_us);
    res->done_ts_us = now_us();
    res->recall10 = recall_at_k_overlap(&topk, worker->gt, task->query_idx, TOPK);
    res->rc = 0;
    return 0;
}

static void *coarse_worker_main(void *arg)
{
    CoarseWorker *worker = (CoarseWorker *)arg;
    ...
    if (worker->init_rc == 0 && worker->use_faiss_coarse) {
        ...
        if (!worker->coarse_labels_buf || !worker->coarse_distances_buf) {
            perror("calloc coarse faiss buffers");
            worker->init_rc = -1;
        }
    }

    if (worker->init_rc == 0 && worker->use_faiss_coarse) {
        if (coarse_search_module_init(&worker->coarse_module,
                                      worker->meta->centroids,
                                      worker->meta->header.dim,
                                      worker->meta->nlist) != 0) {
            fprintf(stderr, "coarse_search_module_init failed for disk %u\n", worker->disk_id);
            worker->init_rc = -1;
        }
    }

    start_barrier_signal_ready(worker->barrier, worker->init_rc);
    if (worker->init_rc != 0 || worker->barrier->fatal_rc != 0) {
        goto cleanup;
    }

    for (uint32_t batch_begin = worker->disk_id * worker->query_batch_size;
         batch_begin < worker->max_queries;
         batch_begin += worker->query_batch_size * NUM_SHARDS) {
        uint32_t batch_end = batch_begin + worker->query_batch_size;
        ...
        for (uint32_t query_idx = batch_begin; query_idx < batch_end; query_idx++) {
            ...
            if (coarse_search_hits(query,
                                   worker->meta,
                                   worker->nprobe,
                                   worker->use_faiss_coarse,
                                   worker->coarse_module,
                                   worker->hits_buf,
                                   worker->coarse_labels_buf,
                                   worker->coarse_distances_buf) != 0) {
                fprintf(stderr, "coarse search failed for query %u on disk %u\n", query_idx, worker->disk_id);
                task_queue_fail(worker->queue, -1);
                start_barrier_fail(worker->barrier, -1);
                goto cleanup;
            }

            size_t bytes = sizeof(PreparedTask) + (size_t)worker->nprobe * sizeof(CoarseHit);
            PreparedTask *task = (PreparedTask *)malloc(bytes);
            if (!task) {
                perror("malloc PreparedTask");
                task_queue_fail(worker->queue, -1);
                start_barrier_fail(worker->barrier, -1);
                goto cleanup;
            }
            task->query_idx = query_idx;
            task->disk_id = worker->disk_id;
            task->coarse_us = now_us() - coarse_begin_us;
            task->enqueue_ts_us = now_us();
            memcpy(task->hits, worker->hits_buf, (size_t)worker->nprobe * sizeof(CoarseHit));

            if (task_queue_push(worker->queue, task) != 0) {
                free(task);
                task_queue_fail(worker->queue, -1);
                start_barrier_fail(worker->barrier, -1);
                goto cleanup;
    ...
    task_queue_close(worker->queue);

cleanup:
    coarse_search_module_destroy(worker->coarse_module);
    worker->coarse_module = NULL;
...
static void *io_worker_main(void *arg)
{
    ...
    for (;;) {
        int fatal_rc = 0;
        PreparedTask *task = task_queue_pop(worker->queue, &fatal_rc);
        if (!task) {
            if (fatal_rc != 0) {
                start_barrier_fail(worker->barrier, fatal_rc);
            }
            break;
        }

        if (run_prepared_query(worker, task) != 0) {
            fprintf(stderr, "I/O scan failed for query %u on disk %u\n", task->query_idx, worker->disk_id);
            free(task);
            task_queue_fail(worker->queue, -1);
            start_barrier_fail(worker->barrier, -1);
            break;
...
static int resolve_worker_cores(const Config *cfg, uint32_t *thread_count, int **cores_out)
{
    if (!cfg || !thread_count || !cores_out) {
        return -1;
    }
    ...
    *cores_out = NULL;
    ...
    if (cfg->cores_spec) {
        ...
        if (cfg->threads_explicit && *thread_count != core_count) {
            fprintf(stderr,
                    "resolve_worker_cores: expected total_threads=%u but --cores count=%u\n",
                    *thread_count,
                    core_count);
            free(*cores_out);
            *cores_out = NULL;
    ...
    if (cfg->base_core >= 0) {
        int *cores = (int *)malloc((size_t)(*thread_count) * sizeof(*cores));
        if (!cores) {
...
static void print_usage(const char *prog)
{
    fprintf(stderr,
            ...
            "\n"
            "Notes:\n"
            "  --threads is the total number of I/O workers. This pipeline version also starts %d coarse workers, one per disk.\n"
            "\n"
            "Built-in paths:\n"
            "  meta=%s\n"
            "  sorted_ids=%s\n"
...
static int parse_args(int argc, char **argv, Config *cfg)
{
    ...
    if (!cfg->disk_traddr[0] || !cfg->disk_traddr[1] || !cfg->disk_traddr[2] ||
        !cfg->disk_traddr[3] || cfg->nprobe == 0 || cfg->threads < NUM_SHARDS ||
...
int main(int argc, char **argv)
{
    ...
    TaskQueue queues[NUM_SHARDS];
    CoarseWorker coarse_workers[NUM_SHARDS];
    IoWorker *io_workers = NULL;
    QueryResult *results = NULL;
    StartBarrier barrier;
    int *thread_cores = NULL;
    uint32_t io_worker_count = 0;
    uint32_t total_thread_count = 0;
    uint32_t created_io_workers = 0;
    uint32_t io_workers_by_disk[NUM_SHARDS] = {0};
    ...
    bool queues_inited[NUM_SHARDS] = {false, false, false, false};
    ...
    memset(coarse_workers, 0, sizeof(coarse_workers));
    memset(&barrier, 0, sizeof(barrier));
    ...
    opts.name = "ivf_baseline_4way_raid_gist_pipeline";
    opts.mem_size = 1024;
    ...
    io_worker_count = cfg.threads;
    total_thread_count = io_worker_count + NUM_SHARDS;
    if (resolve_worker_cores(&cfg, &total_thread_count, &thread_cores) != 0) {
        goto cleanup;
    }
    if (total_thread_count < NUM_SHARDS * 2) {
        fprintf(stderr, "total thread count must be at least %d (4 coarse + 4 io)\n", NUM_SHARDS * 2);
        goto cleanup;
    }
    io_worker_count = total_thread_count - NUM_SHARDS;
    ...
    io_workers = (IoWorker *)calloc(io_worker_count, sizeof(*io_workers));
    if (!io_workers) {
        perror("calloc io_workers");
        goto cleanup;
    }

    for (uint32_t d = 0; d < NUM_SHARDS; d++) {
        if (task_queue_init(&queues[d]) != 0) {
            goto cleanup;
        }
        queues_inited[d] = true;
    }

    if (start_barrier_init(&barrier, total_thread_count) != 0) {
        goto cleanup;
    }
    barrier_inited = true;
    ...
    for (uint32_t d = 0; d < NUM_SHARDS; d++) {
        CoarseWorker *worker = &coarse_workers[d];
        worker->core_id = thread_cores ? thread_cores[d] : -1;
        worker->disk_id = d;
        worker->use_faiss_coarse = strcmp(cfg.coarse_backend, "faiss") == 0;
        ...
        worker->queue = &queues[d];
        worker->barrier = &barrier;

        if (pthread_create(&worker->tid, NULL, coarse_worker_main, worker) != 0) {
            fprintf(stderr, "pthread_create failed for coarse worker %u\n", d);
            start_barrier_fail(&barrier, -1);
            goto join_threads;
        }
    }

    for (uint32_t i = 0; i < io_worker_count; i++) {
        IoWorker *worker = &io_workers[i];
        worker->core_id = thread_cores ? thread_cores[NUM_SHARDS + i] : -1;
        worker->disk_id = i % NUM_SHARDS;
        io_workers_by_disk[worker->disk_id]++;
        worker->ctrlr = disks[worker->disk_id].ctrlr;
        worker->ns = disks[worker->disk_id].ns;
        worker->sector_size = disks[worker->disk_id].sector_size;
        ...
        worker->queue = &queues[worker->disk_id];
        worker->barrier = &barrier;

        if (pthread_create(&worker->tid, NULL, io_worker_main, worker) != 0) {
            fprintf(stderr, "pthread_create failed for io worker %u\n", i);
            start_barrier_fail(&barrier, -1);
            created_io_workers = i;
            goto join_threads;
        }
        created_io_workers = i + 1;
    }

    if (start_barrier_wait_ready(&barrier) != 0) {
        goto join_threads;
    }
    run_begin_us = now_us();
...
join_threads:
    for (uint32_t d = 0; d < NUM_SHARDS; d++) {
        if (coarse_workers[d].tid) {
            pthread_join(coarse_workers[d].tid, NULL);
        }
    }
    for (uint32_t i = 0; i < created_io_workers; i++) {
        pthread_join(io_workers[i].tid, NULL);
    }

    if (barrier.fatal_rc != 0) {
        fprintf(stderr, "query pipeline benchmark failed during worker execution\n");
        goto cleanup;
    }

    {
        ...
        run_wall_us = max_done_ts_us > run_begin_us ? (max_done_ts_us - run_begin_us) : 0;
    }

    printf("[baseline4way-raid-pipeline] disks=%s,%s,%s,%s sector=%u vectors_per_lba=%u nlist=%u nprobe=%u max_queries=%u query_batch_size=%u io_threads=%u coarse_threads=%u io_workers_per_disk=%u,%u,%u,%u max_lbas_per_read=%u coarse_backend=%s\n",
           disks[0].traddr,
           disks[1].traddr,
           disks[2].traddr,
           disks[3].traddr,
           ...
           io_worker_count,
           NUM_SHARDS,
           io_workers_by_disk[0],
           io_workers_by_disk[1],
           io_workers_by_disk[2],
           io_workers_by_disk[3],
           max_lbas_per_read,
           cfg.coarse_backend);
    if (thread_cores) {
        printf("[baseline4way-raid-pipeline] thread_cores=");
        for (uint32_t i = 0; i < total_thread_count; i++) {
            printf(i == 0 ? "%d" : ",%d", thread_cores[i]);
        }
        printf("\n");
    }
    fflush(stdout);
    ...
    if (queries_ran > 0) {
        ...
        printf("[baseline4way-raid-pipeline summary] queries=%u nprobe=%u query_batch_size=%u io_threads=%u coarse_threads=%u io_workers_per_disk=%u,%u,%u,%u total_wall_ms=%.3f qps=%.3f avg_latency_ms=%.3f avg_coarse_ms=%.3f avg_io_ms=%.3f avg_compute_ms=%.3f avg_candidates=%.1f avg_bundles=%.1f avg_recall@%d=%.4f disk_queries=%u,%u,%u,%u\n",
               queries_ran,
               cfg.nprobe,
               cfg.query_batch_size,
               io_worker_count,
        ...
        printf("[baseline4way-raid-pipeline summary] no queries completed\n");
        fflush(stdout);
    }
